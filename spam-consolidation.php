...
$rownum = 1;
$ratesAvgCounter = 0;
$ratesAvgGuardContract12a = 0;
$ratesAvgGuardContract12b = 0;
$ratesAvgMonthly = 0;
$ratesAvgYearly = 0;
$listOfBU = array();
...
if($_SESSION['multi-admin']) {
	$mainspamsql = mysqli_query($conn,"SELECT * FROM bu_mst WHERE id IN (" . implode(',', array_map('intval', $listOfBU)) . ") AND EXPRO = 0 ORDER BY bu");
} else {
...
while($mainspamres = mysqli_fetch_array($mainspamsql))
{
	...
	//$specificspamsql = mysqli_query($conn, "SELECT * FROM spam_mst WHERE bu = ".$mainspamres['id']." AND date_saved < '2020-01-01' ORDER BY date_saved DESC");
	$specificspamres = mysqli_fetch_assoc($specificspamsql);
	$guardTotal = $specificspamres['guard_dc'] + $specificspamres['guard_hg'] + $specificspamres['guard_sg'] + $specificspamres['guard_lg'];
	$guardtable .=	"<tr class='".$rowcolor."' align='center'>" .
						...
						"<td style='background-color:black; padding:0;'></td>".
					//	"<td >".$guardDC."</td>" .
						"<td >".$specificspamres['guard_dc']."</td>" .
					//	"<td >".$guardHG."</td>" .
						"<td >".$specificspamres['guard_hg']."</td>" .
					//	"<td >".$guardSG."</td>" .
						"<td >".$specificspamres['guard_sg']."</td>" .
					//	"<td>".$guardLG."</td>" .
						"<td >".$specificspamres['guard_lg']."</td>" .
						"<td>".$guardIn."</td>" .
					//	"<td>".$guardRel."</td>" .
	...
	$grandtotalGuardDC += $specificspamres['guard_dc'];
	//$grandtotalGuardHG += $guardHG;
	$grandtotalGuardHG += $specificspamres['guard_hg'];
	//$grandtotalGuardSG += $guardSG;
	$grandtotalGuardSG += $specificspamres['guard_sg'];
	//$grandtotalGuardLG += $guardLG;
	$grandtotalGuardLG += $specificspamres['guard_lg'];
	...
	if($specificspamres['rates_contract_per_guard_12a'] > 0)
	{
		$ratesAvgGuardContract12a += $specificspamres['rates_contract_per_guard_12a'];
		$ratesAvgCounter12a++;
	}
	if($specificspamres['rates_contract_per_guard_12b'] > 0)
	{
		$ratesAvgGuardContract12b += $specificspamres['rates_contract_per_guard_12b'];
		$ratesAvgCounter12b++;
	}
	if($specificspamres['rates_tot_contract_per_month'] > 0)
	{
		$ratesAvgMonthly += $specificspamres['rates_tot_contract_per_month'];
		$ratesAvgCounterMonthly++;
	}
	if($specificspamres['rates_contract_per_year'] > 0)
	{
		$ratesAvgYearly += $specificspamres['rates_contract_per_year'];
		$ratesAvgCounterYearly++;
	}	
...
//	$formattedYearly = number_format(($ratesAvgYearly),2,'.',',');
	
	$formattedratesAvgGuardContract12a = number_format(($ratesAvgGuardContract12a / $ratesAvgCounter12a),2,'.',',');
	$formattedratesAvgGuardContract12b = number_format(($ratesAvgGuardContract12b / $ratesAvgCounter12b),2,'.',',');
	$formattedratesAvgMonthly = number_format(($ratesAvgMonthly),2,'.',',');
	$formattedratesAvgYearly = number_format(($ratesAvgYearly),2,'.',',');				
...
$resulttable .=	"<table id='tblSpamRates' class='spamtable' align='center' width='95%' border='1' style=\"border-collapse:collapse; border-spacing:0px; padding:0px; display:none; \" >" .
					...
					"<tr align='center' style='background-color:#FAD8D8;'>" .
						...
						"<td></td>" .
						"<td style='background-color:black; padding:0;' ></td>".
						"<td>".$formattedratesAvgGuardContract12a."</td>" .
						"<td>".$formattedratesAvgGuardContract12b."</td>" .
						"<td></td>" .
						"<td></td>" .
						"<td style='background-color:black; padding:0;' ></td>".
						"<td>". $formattedratesAvgMonthly ."</td>" .						
						"<td>". $formattedratesAvgYearly ."</td>" .
						"<td style='background-color:black; padding:0;' ></td>".
					"</tr>" .
					$ratestable .
					"<tr align='center' style='background-color:#FAD8D8;'>" .
