...
elseif ($type == "evaluateAgency") {
	...
	while ($evaluateAgency = mysqli_fetch_assoc($result)) {
		$getLegalScore = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(score) as legalScore FROM `bidding_specific` INNER JOIN bidding_template_item ON bidding_specific.template_id = bidding_template_item.id WHERE bidding_template_item.category = 'Legal' and bidding_specific.bidding_id =  " . $id . " and bidding_specific.agency_id = " . $evaluateAgency['id']));
		$getTechnicalScore = mysqli_fetch_array(mysqli_query($conn, "SELECT SUM(score) as technicalScore FROM `bidding_specific` INNER JOIN bidding_template_item ON bidding_specific.template_id = bidding_template_item.id WHERE bidding_template_item.category = 'Technical' and bidding_specific.bidding_id =  " . $id . "  and bidding_specific.agency_id = " . $evaluateAgency['id']));
