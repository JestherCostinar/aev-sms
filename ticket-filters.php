...
if($_SESSION['level'] == 'Super Admin' || $rowuser['level'] == 'Custom Admin')
{
	...
	$locsql = mysqli_query($conn, "SELECT * FROM location_mst WHERE bu IN (" . implode(',', array_map('intval', $listOfBU)) . ") ORDER BY bu, location_code");
	...
	$guardsql = mysqli_query($conn, "SELECT * FROM guard_personnel WHERE bu IN (" . implode(',', array_map('intval', $listOfBU)) . ") ORDER BY lname");
...
elseif($filtertoken == "monitoringgraph")
{
	$btnFilter =	'<td align="center" colspan="100%">
						<button onclick="generateReportTable2();" class="redbutton" style="width:150px;">Generate Graph</button>						
					</td>';
	$txtticketid = "";
}
...
$divcontent =	'<table align="center" >
					...
					<tr>
						...
						<td align="left">
							<select id="selSearchTicketLevel" name="selSearchTicketLevel">
								<option>1</optionn>
								<option>2</optionn>
								<option>3</optionn>
								<option>4</optionn>
