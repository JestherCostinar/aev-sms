...
$displayBUName = 'BU Security Head';
...
$rowclass2 = "";
$locationarray = array();
$locsql = mysqli_query($conn, "SELECT * FROM location_mst WHERE bu IN (" . implode(',', array_map('intval', $listOfBU)) . ") ORDER BY location_code");
...
$guardsdatalist2 = "";
$guardsarray = array();
$checkexprosql = mysqli_query($conn, "SELECT * FROM bu_mst WHERE id IN (" . implode(',', array_map('intval', $listOfBU)) . ")");
...
$guardsql = mysqli_query($conn, "SELECT * FROM guard_personnel WHERE bu IN (" . implode(',', array_map('intval', $listOfBU)) . ") ORDER BY lname, fname, mname");
...
$oicnumber = 1;
$oicrow = 1;
$oicsql = mysqli_query($conn, "SELECT * FROM oic_mst WHERE bu IN (" . implode(',', array_map('intval', $listOfBU)) . ") ORDER BY bu, lname");
...
$userstable = "";
$usersnumber = 1;
$userssql = mysqli_query($conn, "SELECT users_mst.*, bu_mst.bu as bu_name FROM users_mst INNER JOIN bu_mst ON users_mst.bu = bu_mst.id  WHERE users_mst.bu IN (" . implode(',', array_map('intval', $listOfBU)) .") ORDER BY users_mst.lname");
...
{
	$guardbudatalist .= "<option value=\"".$guardbulistres['id']."\">".$guardbulistres['bu']."</option>";
	$counter++;
}

$bulistsql = mysqli_query($conn, "SELECT * FROM bu_mst WHERE id IN (" . implode(',', array_map('intval', $listOfBU)) . ")");
...
$exprochecksql = mysqli_query($conn, "SELECT * FROM bu_mst WHERE id IN (" . implode(',', array_map('intval', $listOfBU)) . ")");
											...
											<td width="50%">
												...
												<table align="center">
													<tr>
														...
														<td>
																<select id="selRGBU2" name="selRGBU2">
																<option value="0">All Business Unit</option>		
																'.$budatalist.'
															</select>
												...
												<table align="center">
													<tr>
														...
														<td>
														<select id="selRGBU3" name="selRGBU3">
																<option value="0">All Business Unit</option>		
																'.$budatalist.'
															</select>                                       
														</td>
		...
		if((isset($_POST['txtOrigin'])) && !empty($_POST['txtOrigin']))
		{
			...
			if($oic==1)
			{
				...
				// {					
					...
					$mail = send_mail3($to,$subject,$mainbody,$upload1,$upload2,$upload3,$to2);
					if($mail)
						{
							$mailtest = "SUCCESS";
						}
...
//				}
	...
	elseif((isset($_POST['txtIncidentDisposition'])) && !empty($_POST['txtIncidentDisposition']))
	{
		...
		$mainbody = '<table border="1" width="75%" align="center" style="font-family: Calibri, Candara, Segoe, Optima, Arial, sans-serif; border-collapse:collapse;">
			...
			$mail = send_mail_disposition($to,$subject,$mainbody,$to2);
			if($mail)
			{
				$mailtest = "SUCCESS";
			}
...
$guardsql2 = mysqli_query($conn, "SELECT * FROM guard_personnel WHERE bu IN (" . implode(',', array_map('intval', $listOfBU)) . ") ORDER BY bu, status, lname");
$guardstable = "";
$guardnum = 1;
$guardrow = 0;
...
$incidentcount = 0;
$incidentcountsql = mysqli_query($conn,"SELECT COUNT(id) AS Incident_Count FROM ticket WHERE ticket_type = 1 AND bu IN (" . implode(',', array_map('intval', $listOfBU)) . ") AND MONTH(dateadded) = MONTH(now()) AND YEAR(dateadded) = YEAR(now());");
...
eval('$body = "' . fetch_template('multi-bu') . '";');
