...
if(($newtype == "Inc") || ($newtype == "Act"))
{
	...
	elseif(($_SESSION['level'] == 'Admin') && (!$_SESSION['multi-admin']))
	{
		if($newtype == "Inc")
		{
			$activitysql = mysqli_query($conn, "select * from ticket where ((dateadded > DATE_SUB(now(), INTERVAL 4 DAY)) OR (is_open = 1)) AND bu = ".$bu." AND ticket_type = 1 order by datesubmitted desc");
			$incidenttable = "";
			while($actres = mysqli_fetch_assoc($activitysql))
			{
				$logid = "ticket" . $actres['id'];
		...
		elseif($newtype == "Act")
		{
			$activitysql = mysqli_query($conn, "select * from ticket where ((dateadded > DATE_SUB(now(), INTERVAL 4 DAY)) OR (is_open = 1)) AND bu = ".$bu." AND ticket_type = 2 order by datesubmitted desc");
			$activitytable = "";			
			while($actres = mysqli_fetch_assoc($activitysql))
			{
				$logid = "ticket" . $actres['id'];
...
elseif($newtype == "Guard")
{
	if($_SESSION['level'] == 'Admin')
	{
		...
		$guardsql2 = mysqli_query($conn, "SELECT * FROM guard_personnel WHERE bu = ".$guardbu." ORDER BY CASE bu WHEN ". $guardbu ." THEN 1 ELSE 2 END, bu, status, lname");
		$guardstable = "";
		$guardnum = 1;
		$guardrow = 0;
